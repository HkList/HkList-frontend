    ...
    <t-form :data="addAccountInput" :rules="formRules" @submit="submitForm" :labelWidth="130">
      <t-form-item label="账号类型">
        <t-select v-model="addAccountType">
          <t-option label="Cookie" value="cookie" />
          <t-option label="企业" value="enterprise_cokie" />
          <t-option label="开放平台" value="open_platform" />
          <t-option label="下载卷" value="download_ticket" />
        </t-select>
      </t-form-item>

      <template v-if="addAccountType === 'cookie' || addAccountType === 'enterprise_cokie'">
        <t-form-item label="Cookie" name="cookie">
      ...
      <template v-else-if="addAccountType === 'download_ticket'">
        <t-form-item label="Surl" name="surl" help="分享链接">
          <t-input v-model="addAccountInput.surl" />
        </t-form-item>

        <t-form-item label="Pwd" name="pwd" help="链接密码">
          <t-input v-model="addAccountInput.pwd" />
        </t-form-item>

        <t-form-item label="Dir" name="dir" help="盘内路径">
          <t-input v-model="addAccountInput.dir" />
        </t-form-item>

        <t-form-item label="SaveCookie" name="save_cookie" help="企业号CK">
          <t-textarea v-model="addAccountInput.save_cookie" />
        </t-form-item>

        <t-form-item label="DownloadCookie" name="download_cookie" help="普通号CK">
